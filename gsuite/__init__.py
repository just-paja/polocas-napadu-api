...
def get_perm_method(action, access):
    if action == MODEL_ACTION_VIEW:
        return PERM_ENSURE if access in [MODEL_ACCESS_RO, MODEL_ACCESS_RW] else PERM_DELETE
...
def get_models_permissions(apps, group):
    ...
    for perm in group['perms']:
        models = get_all_model_names(apps) if perm['models'] == MODELS_ALL else perm['models']
        for model in models:
