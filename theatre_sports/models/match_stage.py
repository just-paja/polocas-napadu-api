...
class MatchStage(TimeStampedModel):
    ...
    match = ForeignKey(
        "Match", on_delete=CASCADE, related_name="stages", verbose_name=_("Match"),
    )
    game = ForeignKey(
        "games.Game",
        blank=True,
    ...
    type = PositiveIntegerField(choices=STAGE_CHOICES, verbose_name=_("Stage Type"),)
    ...
    def __str__(self):
        return "%s, %s" % (self.get_type_display(), self.match.show,)
    ...
    def get_duration(self):
        next_stage = (
            self.match.stages.filter(created__gt=self.created)
            .order_by("created")
            .first()
        )
        if next_stage:
